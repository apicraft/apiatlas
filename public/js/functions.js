    ...
    var preloads = {
        'vis': function(){
            ...
            $.get("./templates/vis_resource.ejs", function(t){
                ...
                var max_p = 100;
                //verbs
                $("#resources").append(title.render({"color": "blue", "title": "verbs"}));
                for(i in http.verbs){
                    var p = randomFromInterval(0,max_p);
                    $("#resources").append(template.render({"type": "verb", "color": "blue", "percent": p, "percent_display": p + "%", "name": http.verbs[i],"votes": randomFromInterval(0,200) }) );
                    
                }
                //headers
                $("#resources").append(title.render({"color": "green", "title": "headers"}));
                ...
                //codes
                $("#resources").append(title.render({"color": "yellow", "title": "codes"}));
                for(i in http.codes){
                    var p = randomFromInterval(0,max_p);
